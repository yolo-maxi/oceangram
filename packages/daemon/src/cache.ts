...
export class Cache {
  ...
  getMessages(dialogId: string, limit: number, offsetId?: number): MessageInfo[] {
    const stmt = offsetId
      ? this.db.prepare('SELECT * FROM messages WHERE dialog_id = ? AND id < ? ORDER BY date DESC LIMIT ?')
      : this.db.prepare('SELECT * FROM messages WHERE dialog_id = ? ORDER BY date DESC LIMIT ?');

    const rows = offsetId
      ? stmt.all(dialogId, offsetId, limit) as any[]
      : stmt.all(dialogId, limit) as any[];
  ...
  upsertMessages(dialogId: string, messages: MessageInfo[]): void {
    ...
    const tx = this.db.transaction((msgs: MessageInfo[]) => {
      for (const m of msgs) {
        stmt.run(
          m.id, dialogId, m.senderId || null, m.senderName || null,
          m.text || null, m.timestamp || null,
          m.isEdited ? Math.floor(Date.now() / 1000) : null,
          m.replyToId || null, m.mediaType || null,
          m.isOutgoing ? 1 : 0, JSON.stringify(m),
        );
      }
    });
    tx(messages);
  ...
  getDialogs(limit: number): DialogInfo[] {
    const rows = this.db.prepare('SELECT * FROM dialogs ORDER BY last_message_time DESC LIMIT ?').all(limit) as any[];
  ...
  upsertDialogs(dialogs: DialogInfo[]): void {
    ...
    const tx = this.db.transaction((ds: DialogInfo[]) => {
      for (const d of ds) {
        stmt.run(
          d.id, d.chatId || d.id, d.topicId || null, d.name || null,
          d.type || null, d.lastMessage || null, d.lastMessageTime || null,
          d.unreadCount || 0, d.isForum ? 1 : 0, d.groupName || null,
          d.topicName || null, d.hasPhoto ? 1 : 0, JSON.stringify(d),
...
function rowToMessageInfo(row: any): MessageInfo {
  if (row.raw) {
    try {
      ...
      return { ...parsed, id: row.id, text: row.text ?? parsed.text, isEdited: row.edit_date ? true : parsed.isEdited };
    } catch { /* fall through */ }
  }
  return {
    id: row.id, senderId: row.from_id || '', senderName: row.sender_name || '',
    text: row.text || '', timestamp: row.date || 0, isOutgoing: row.is_outgoing === 1,
    mediaType: row.media_type || undefined, replyToId: row.reply_to || undefined,
...
function rowToDialogInfo(row: any): DialogInfo {
  if (row.raw) {
    try {
      const parsed = JSON.parse(row.raw);
      return { ...parsed, id: row.id, unreadCount: row.unread_count ?? parsed.unreadCount, lastMessageTime: row.last_message_time ?? parsed.lastMessageTime };
    } catch { /* fall through */ }
  }
  return {
    id: row.id, chatId: row.chat_id || row.id, name: row.name || 'Unknown',
    lastMessage: row.last_message || '', lastMessageTime: row.last_message_time || 0,
    unreadCount: row.unread_count || 0, isForum: row.is_forum === 1,
    groupName: row.group_name || undefined, topicName: row.topic_name || undefined,
    topicId: row.topic_id || undefined, hasPhoto: row.has_photo === 1,
