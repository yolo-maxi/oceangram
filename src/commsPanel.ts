...
export class CommsPanel {
  public static currentPanel: CommsPanel | undefined;
  private static readonly viewType = 'oceangram.comms';
  ...
  public static createOrShow(context: vscode.ExtensionContext) {
    ...
    if (CommsPanel.currentPanel) {
      CommsPanel.currentPanel.panel.reveal(vscode.ViewColumn.One);
      return;
    }
    ...
    CommsPanel.currentPanel = new CommsPanel(panel, context);
  }
  ...
  private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext) {
    ...
    this.panel.onDidDispose(() => {
      CommsPanel.currentPanel = undefined;
...
.empty { text-align: center; padding: 32px; opacity: 0.5; font-size: 14px; }
...
<div id="chatListView" class="view active">
  ...
  <div class="search-bar">
    <input type="text" id="searchInput" placeholder="Search chats to pin..." />
  </div>
  <div class="content" id="chatList">
    <div class="loading">Connecting to Telegram...</div>
  </div>
...
function showView(view) {
  ...
  if (view === messageView) backBtn.style.display = 'block';
}
...
function renderDialogs(dialogs, container, showPinBtn) {
  ...
  container.querySelectorAll('.chat-item').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      openChat(el.dataset.id, el.dataset.name);
    });
  });
